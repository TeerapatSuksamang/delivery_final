        ...
        <div class="row justify-content-center mt-5">
            <div class="col-md-6">
                <form action="profile_edit_db.php" class="card shadow p-4" method="post" enctype="multipart/form-data">
                    <h1 class="text-center mb-5">แก้ไขข้อมูลร้านอาหาร</h1>
                    <?php
                        $select = mysqli_query($conn, "SELECT * FROM `restaurant` WHERE `res_id` = '".$_SESSION['res_id']."' ");
                        $row = mysqli_fetch_array($select);
                    ?>

                    <input type="hidden" name="res_id" value="<?php echo $row['res_name']; ?>">

                    <label for="" class="form-lable">ชื่อร้านอาหาร</label>
                    <input type="text" class="form-control mb-4" name="res_name" value="<?php echo $row['res_name']; ?>" required>
                    ...
                    <input type="text" class="form-control mb-4" name="res_type" value="<?php echo $row['res_type']; ?>" readonly>
