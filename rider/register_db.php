<?php
    ...
    if(isset($_POST['submit'])){
        ...
        $select = mysqli_query($conn, "SELECT * FROM `rider` WHERE `username` = '$username' ");
        
        if(mysqli_num_rows($select) > 0){
            echo "<script>alert('ชื่อผู้ใช้ซ้ำ กรุณาเปลี่ยนใหม่'); window.location = 'register.php';</script>";
        } else {
            $sql = mysqli_query($conn, "INSERT INTO `rider`(`firstname`, `lastname`, `img`, `username`, `password`, `address`, `phone`, `status`) 
